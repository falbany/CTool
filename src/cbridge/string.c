...
// Internal Helper
// ============================================================================
static void impl_append_len(string_t* self, const char* str, size_t len) {
...
static void impl_free(string_t* self) {
    if (self) {
...
static void impl_clear(string_t* self) {
    if (self && self->data) {
        self->length  = 0;
        self->data[0] = '\0';
    }
...
static bool impl_empty(const string_t* self) { return !self || self->length == 0; }

static size_t impl_length(const string_t* self) { return self ? self->length : 0; }

static const char* impl_c_str(const string_t* self) { return (self && self->data) ? self->data : ""; }

// ============================================================================
// Modifiers
// ============================================================================
static void impl_append(string_t* self, const char* suffix) {
    if (suffix) impl_append_len(self, suffix, strlen(suffix));
}

static void impl_insert(string_t* self, size_t pos, const char* str) {
    if (!self || !str) return;
...
static void impl_erase(string_t* self, size_t pos, size_t len) {
...
static void impl_trim(string_t* self) {
...
// Operations
// ============================================================================
static int impl_compare(const string_t* self, const char* other) {
...
static bool impl_starts_with(const string_t* self, const char* prefix) {
...
static bool impl_ends_with(const string_t* self, const char* suffix) {
...
static size_t impl_find(const string_t* self, const char* target, size_t start_pos) {
    if (!self || !target || start_pos >= self->length) return CBRIDGE_STR_NPOS;
    const char* match = strstr(self->data + start_pos, target);
    if (match) {
        return (size_t)(match - self->data);
    }
    return CBRIDGE_STR_NPOS;
}

static string_t* impl_substr(const string_t* self, size_t pos, size_t len) {
 ...
 */
static string_t* impl_create_len(const char* str, size_t len) {
    string_t* self = (string_t*)malloc(sizeof(string_t));
    if (!self) return NULL;
    self->length   = len;
    self->capacity = len;
...
static string_parts_t impl_split(const string_t* self, const char* delimiter) {
    string_parts_t result = {NULL, 0};
    ...
    if (!delimiter || delimiter[0] == '\0') {
        result.count = 1;
        result.parts = (string_t**)calloc(1, sizeof(string_t*));
        if (!result.parts) return (string_parts_t){NULL, 0};
        result.parts[0] = impl_create_len(self->data, self->length);
        if (!result.parts[0]) {
            free(result.parts);
            return (string_parts_t){NULL, 0};
        }
        return result;
    }
    ...
    result.parts = (string_t**)calloc(part_count, sizeof(string_t*));
    if (!result.parts) return (string_parts_t){NULL, 0};
...
/**
 * @brief Frees a string_parts_t result: deallocates every part and the parts array.
 */
...
// ============================================================================
const struct cbridge_string_namespace cbridge_string = {.create         = impl_create,
                                                        .free           = impl_free,
