 ...
 * functions declared in `cbridge_net.h`. It uses Winsock for Windows and standard
 ...
 * @see cbridge_net_namespace::init
 */
 ...
 * @see cbridge_net_namespace::cleanup
 */
 ...
 * @see cbridge_net_namespace::connect
 */
static cbridge_socket_t impl_connect(const char* host, int port, cbridge_net_proto_t proto) {
    struct addrinfo hints, *res;
    int             sock_type = (proto == CBRIDGE_NET_TCP) ? SOCK_STREAM : SOCK_DGRAM;
    char            port_str[16];
    ...
    cbridge_socket_t sock = (cbridge_socket_t)socket(res->ai_family, res->ai_socktype, res->ai_protocol);
 ...
 * @see cbridge_net_namespace::send
 */
static int impl_send(cbridge_socket_t sock, const char* data, size_t len) { return send(sock, data, (int)len, 0); }
 ...
 * @see cbridge_net_namespace::close
 */
static void impl_close(cbridge_socket_t sock) { closesocket(sock); }

/**
 * @brief The global instance of the networking function table.
 */
const struct cbridge_net_namespace cbridge_net =
