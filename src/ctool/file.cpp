...
namespace ctool {
    namespace file {
        ...
        std::string readAll(const std::string& path) {
            ...
            return ctool::str::findAndReplace(content, "\r", "");
        }
        ...
        std::string getParameter(const std::string& filename, const std::string& key, const std::string& separator) {
            ...
            while (std::getline(file, line)) {
                ...
                if (sepPos != std::string::npos) {
                    std::string currentKey = ctool::str::trim(line.substr(0, sepPos));
                    if (currentKey == key) {
                        return ctool::str::trim(line.substr(sepPos + separator.length()));
                    }
                }
            }
            return "";
