...
The `/net` module provides a C++11 object-oriented interface for TCP networking. It abstracts platform-specific socket code (Winsock/POSIX) into manageable classes.
...
2. It wraps that socket into a `std::unique_ptr<ctool::net::TcpClient>`.
...
ctool::net::TcpClient client;

if (client.connect("127.0.0.1", 8080) == ctool::net::Status::Ok) {
