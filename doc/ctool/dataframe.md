...
- **Dual-Mode API**: 
  - **Creation**: Data can be loaded from external sources using **Free Functions** (`ctool::data::fromCSV`, `fromArray2D`).
...
The module provides a complete pipeline for data ingestion, manipulation, and export, with tight integration with the `/num` (NumPy-like) module.

#### NumArray Integration (High Performance)
The module supports direct conversion from `DataFrame` to `ctool::num::NumArray`, leveraging contiguous memory layouts for high-performance numerical pipelines.
...
int main() {
    ...
    ctool::data::DataFrame df = ctool::data::fromCSV("sales_data.csv");

    // 2. FILTER Data
    // Get rows where 'Quantity' > 10
    auto filtered = ctool::data::filterGt(df, "Quantity", 10);

    // 3. CONVERT to High-Performance Matrix
    // Extracts 'Price' and 'Discount' into a contiguous NumArray
    ctool::num::NumArray<double> matrix = filtered.toNumArray({"Price", "Discount"});

    // 4. PROCESS with NumArray Ufuncs (Vectorized Math)
    // Apply 10% discount: Discount = Price * 0.1
    ctool::num::NumArray<double> discounts = ctool::num::mul(matrix, 0.1); // Note: using ctool::num::mul if available, or manual
    
    // Example using element-wise multiplication:
    ctool::num::NumArray<double> price_col = matrix.sliceRow(0); // Conceptual
    // Let's use element-wise logic:
    ctool::num::NumArray<double> profit = matrix + matrix; // Just a demo of + operator

    // Better Example: Calculate Total Revenue (Price * Quantity)
    // Assuming 'Quantity' is column 0 and 'Price' is column 1
    ctool::num::NumArray<double> revenue = matrix; 
    // (In a real scenario, you'd perform row-wise math or use broadcasting)
    
    // 5. EXPORT Results back to DataFrame and CSV
    auto result_df = ctool::data::DataFrame::fromNumArray(revenue, {"Total_Revenue"});
...
2.  **`fromArray2D(array, names)`**
    *   Converts a legacy `ctool::array::Array2D` into a DataFrame.
    *   Useful for migrating data from older grid-based systems.

3.  **`fromNumArray(array, names)`**
    *   Converts a high-performance `ctool::num::NumArray` into a DataFrame.
...
```cpp
std::ostream& operator<<(std::ostream& os, const ctool::data::DataFrame& df);
```
...
auto midRange = ctool::data::filter(df, "Score", 85.0, ctool::data::CompareOp::GE);
```
...
ctool::array::Array2D<double> legacy_data = {{1.1, 2.2}, {3.3, 4.4}};
auto df2 = ctool::data::fromArray2D(legacy_data, {"ColA", "ColB"});

// Load from high-performance NumArray
ctool::num::NumArray<double> num_data = {{5.0, 6.0}};
auto df3 = ctool::data::fromNumArray(num_data, {"X", "Y"});
```
...
auto roots = ctool::num::sqrt(matrix);

// Calculate sine of values
auto sine_vals = ctool::num::sin(matrix);
...
// 5. Reductions
double total_temp = ctool::num::sum(matrix); // Sum of all elements
double avg_pressure = ctool::num::mean(matrix); // Average
```
...
if (ctool::data::isNull(df.at(0, "ID"))) {
