# ct_unit: Engineering Notation & Unit Utilities

## 📖 Overview
The `ct_unit` module provides utilities for handling engineering notation (SI prefixes), robust string parsing with units, and common engineering conversions (like temperature). It is designed to simplify the representation and input of physical quantities in scientific and engineering applications.
...
// Standard formatting
std::string s1 = ct::unit::format(1200.0, 1, "Hz");   // "1.2 kHz"
std::string s2 = ct::unit::format(0.0000015, 2, "A"); // "1.50 uA"

// Without units
std::string s3 = ct::unit::format(1500000.0, 0);      // "2 M" (rounded)
```
...
```cpp
double v1 = ct::unit::parse("1.5kV");      // 1500.0
double v2 = ct::unit::parse("100 mA");     // 0.1
double v3 = ct::unit::parse("2.2uF");      // 0.0000022
double v4 = ct::unit::parse("470");        // 470.0 (no prefix)
```

### 3. Temperature Conversions
```cpp
double kelvin  = ct::unit::cToK(25.0);     // 298.15
double celsius = ct::unit::kToC(0.0);      // -273.15
```
