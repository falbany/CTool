...
The `/json` module provides a lightweight, dependency-free way to handle JSON strings in C++11. It is optimized for flat configuration objects and simple data exchange.
...
std::string rawJson = ctool::json::stringify(config);
...
std::string status = ctool::json::getValue(response, "status");
...
std::string compact = ctool::json::minify(pretty);
```
